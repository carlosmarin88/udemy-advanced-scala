...
object JVMConcurrencyProblems {
  ...
  def buySafe(bankAccount: BankAccount, thing: String, price: Int): Unit = {
    bankAccount.synchronized { // doesn't allow multiple threads to run the critical section AT THE SAME TIME
  ...
  */
   ...
   *       -> threads 3
   *          .......
   *   each threads prints "hello from threads $i"
